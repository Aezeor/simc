...
ground_aoe_params_t::ground_aoe_params_t()
  : target_( nullptr ),
    ...
    action_( nullptr ),
    pulse_time_( 1_s ),
    start_time_( timespan_t::min() ),
    duration_( 0_ms ),
    expiration_pulse_( NO_EXPIRATION_PULSE ),
    n_pulses_( 0 ),
    expiration_cb_( nullptr )
{}
...
ground_aoe_event_t::ground_aoe_event_t( player_t* p, const ground_aoe_params_t* param, action_state_t* ps,
                                        bool immediate_pulse )
    ...
    current_pulse( 1 )
{
  ...
  assert( params->pulse_time() > timespan_t::zero() && "Pulse time for ground_aoe_event_t must be a positive value" );
  assert( params->start_time() >= timespan_t::zero() && "Start time for ground_aoe_event must be defined" );
...
timespan_t ground_aoe_event_t::_pulse_time( const ground_aoe_params_t* params, const player_t* p, bool clamp )
{
  ...
  {
    case ground_aoe_params_t::SPELL_CAST_SPEED:  tick *= p->cache.spell_cast_speed();  break;
    case ground_aoe_params_t::SPELL_HASTE:       tick *= p->cache.spell_haste();       break;
    case ground_aoe_params_t::AUTO_ATTACK_SPEED: tick *= p->cache.auto_attack_speed(); break;
    case ground_aoe_params_t::ATTACK_HASTE:      tick *= p->cache.attack_haste();      break;
    default:                                     break;
  }
