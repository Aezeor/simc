...
public:
  ...
  struct talents_t
  {
    ...
    struct
    {
      ...
      // Apex
      player_talent_t forbidden_knowledge_1;  // NYI
      player_talent_t forbidden_knowledge_2;  // NYI
      player_talent_t forbidden_knowledge_3;  // NYI
    } unholy;
    ...
    struct
    {
      ...
      player_talent_t unholy_armaments;  // NYI
    ...
    struct
    {
      ...
      player_talent_t thrill_of_blood; // NYI
  ...
  struct pet_spells_t
  {
    ...
    // Lesser Ghouls
    const spell_data_t* putrefy_buff;
    const spell_data_t* putrefy_damage;
  ...
  struct pets_t
  {
    ...
    spawner::pet_spawner_t<pets::lesser_ghoul_pet_t, death_knight_t> lesser_ghoul_db;
    ...
    pets_t( death_knight_t* p )
      : ghoul_pet( "ghoul", p ),
        ...
        lesser_ghoul_db( "lesser_ghoul_db", p ),
...
struct lesser_ghoul_pet_t final : public base_ghoul_pet_t
{
  ...
  struct putrefy_buff_t final : public buff_t
  {
    ...
    {
      return debug_cast<lesser_ghoul_pet_t*>( this->player );
    }
    ...
    {
      buff_t::start( s, v, d );
      if ( pet()->expiration->remains() < ( dk()->pet_spell.putrefy_buff->duration() + 1_s ) )
      {
        // Ghoul cant expire while putrefy is active, so extend duration
        timespan_t diff = ( dk()->pet_spell.putrefy_buff->duration() + 1_s ) - pet()->expiration->remains();
        pet()->adjust_duration( diff );
      }
    }
  ...
  void init_base_stats() override
  {
    ...
    owner_coeff.ap_from_ap = 0.4664; // TODO: CHECK ME
  }
...
struct summon_lesser_fs_ghoul_t : public death_knight_summon_spell_t
{
  summon_lesser_fs_ghoul_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_lesser_ghoul )
  {
    background = true;
  }
  void execute() override
...
struct summon_lesser_db_ghoul_t : public death_knight_summon_spell_t
{
  summon_lesser_db_ghoul_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_lesser_ghoul )
  {
    background = true;
  }
  ...
  {
    death_knight_summon_spell_t::execute();
    p()->pets.lesser_ghoul_db.spawn();
  }
};

struct summon_army_of_the_dead_ghoul_t : public death_knight_summon_spell_t
{
  summon_army_of_the_dead_ghoul_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_army_ghoul )
  {
    background = true;
  }
...
struct summon_army_magus_t : public death_knight_summon_spell_t
{
  summon_army_magus_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_magus )
  {
    background = true;
  }
...
struct summon_dt_magus_t : public death_knight_summon_spell_t
{
  summon_dt_magus_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_magus )
  {
    background = true;
  }
...
struct summon_reanimation_magus_t : public death_knight_summon_spell_t
{
  summon_reanimation_magus_t( std::string_view n, death_knight_t* p )
    : death_knight_summon_spell_t( n, p, p->spell.summon_magus )
  {
    background = true;
  }
...
struct exterminate_t final : public death_knight_spell_t
{
  ...
  double composite_da_multiplier( const action_state_t* state ) const override
  {
    ...
    if ( empowered )
    {
      const double multiplier_effect = p()->specialization() == DEATH_KNIGHT_FROST ? p()->talent.deathbringer.echoing_fury->effectN( 2 ).percent()
...
struct dark_transformation_t : public death_knight_spell_t
{
      ...
      summon_magus( nullptr )
  {
    ...
    if ( p->talent.unholy.magus_of_the_dead.ok() )
    {
      ...
      summon_magus = get_action<summon_dt_magus_t>( "summon_dt_magus", p );
    }
  }
...
struct army_of_the_dead_t final : public death_knight_summon_spell_t
{
  struct summon_army_event_t : public event_t
  {
    ...
    void execute() override
    {
      summon_army_of_the_dead_ghoul_t* summon_ghoul_cast = debug_cast<summon_army_of_the_dead_ghoul_t*>( summon_ghoul );
      ...
      summon_gargoyle( nullptr )
  {
    ...
    if ( p->talent.unholy.magus_of_the_dead.ok() )
    {
      ...
      summon_magus = get_action<summon_army_magus_t>( "summon_army_magus", p );
    }
  ...
  void execute() override
  {
    ...
    if ( !p()->in_combat && precombat_time > 0 )
    {
      ...
      while ( duration_penalty >= 0_s && n_ghoul < 8 )
      {
        ...
        auto pet =
            p()->pets.lesser_ghoul_army.spawn( summon_duration + ( n_ghoul < 3 ? 1_s / n_ghoul : 0_s ) - duration_penalty, 1 )
                .front();
    ...
    if ( p()->talent.unholy.magus_of_the_dead.ok() )
    {
      summon_army_magus_t* summon_magus_cast = debug_cast<summon_army_magus_t*>( summon_magus );
      summon_magus_cast->set_duration( summon_duration );
      summon_magus->execute();
    }
    ...
    if ( p()->talent.unholy.raise_abomination.ok() )
    {
      ...
      summon_abomination_cast->set_duration( p()->spell.summon_abomination->duration() -
                                             timespan_t::from_seconds( precombat_time ) );
    ...
    if ( p()->talent.unholy.summon_gargoyle.ok() )
    {
      ...
      summon_gargoyle_cast->set_duration( p()->spell.summon_gargoyle->duration() -
                                          timespan_t::from_seconds( precombat_time ) );
...
struct raise_dead_t final : public death_knight_summon_spell_t
{
  raise_dead_t( death_knight_t* p, std::string_view options_str )
    : death_knight_summon_spell_t(
          "raise_dead", p, p->spec.raise_dead->ok() ? p->spec.raise_dead : p->talent.raise_dead )
  {
    parse_options( options_str );
...
struct wound_spender_base_t : public death_knight_melee_attack_t
{
  wound_spender_base_t( std::string_view name, death_knight_t* p, const spell_data_t* spell )
    : death_knight_melee_attack_t( name, p, spell ),
      summon_ghoul( get_action<summon_lesser_fs_ghoul_t>( "summon_fs_ghoul", p ) )
  {
    weapon = &( player->main_hand_weapon );
  }
  ...
  if ( is_vampiric )
  {
    ...
    if ( talent.sanlayn.unending_misery.ok() && t == target)
    {
      if ( specialization() == DEATH_KNIGHT_UNHOLY )
        extension += talent.sanlayn.unending_misery->effectN( 2 ).time_value();
      else if ( specialization() == DEATH_KNIGHT_BLOOD )
        extension += talent.sanlayn.unending_misery->effectN( 4 ).time_value();
    }
...
std::unique_ptr<expr_t> death_knight_t::create_expression( std::string_view name_str )
{
  ...
  if( util::str_compare_ci( splits[ 0 ], "lesser_ghoul" ) && splits.size() > 1 )
  {
...
void death_knight_t::create_pets()
{
  ...
  if ( specialization() == DEATH_KNIGHT_UNHOLY )
  {
    ...
    {
      pets.dt_magus.set_creation_callback(
          []( death_knight_t* p ) { return new pets::magus_pet_t( p, "dt_magus" ); } );
    ...
    if ( talent.unholy.doomed_bidding.ok() )
    {
      ...
      pets.lesser_ghoul_db.set_creation_callback(
          []( death_knight_t* p ) { return new pets::lesser_ghoul_pet_t( p, "db_ghoul" ); } );
      pets.lesser_ghoul_db.set_default_duration( doomed_bidding_duration );
    }
...
void death_knight_t::init_spells()
{
  ...
  talent.unholy.blighted         = find_talent_spell( talent_tree::SPECIALIZATION, "Blighted" );
...
void death_knight_t::spell_lookups()
{
  ...
  // Lesser Ghoul
  pet_spell.putrefy_buff   = conditional_spell_lookup( talent.unholy.doomed_bidding.ok(), 1244030 );
  pet_spell.putrefy_damage = conditional_spell_lookup( talent.unholy.doomed_bidding.ok(), 390220 );
...
void death_knight_t::create_buffs()
{
  ...
  buffs.death_and_decay = make_fallback<death_and_decay_buff_t>(
      spec.death_and_decay->ok(), this, "death_and_decay", spell.dnd_buff );
  ...
  // Deathbringer
  buffs.empowered_soul           = make_fallback( sets->has_set_bonus( HERO_DEATHBRINGER, TWW3, B4 ), this, "empowered_soul", talent.deathbringer.echoing_fury->effectN( 3 ).trigger() );
