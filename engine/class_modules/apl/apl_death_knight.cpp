...
void unholy( player_t* p )
{
  ...
  cooldowns->add_action( "army_of_the_dead,if=(variable.st_planning|variable.adds_remain)&!talent.summon_gargoyle&!talent.gift_of_the_sanlayn|talent.summon_gargoyle&runic_power>=30|debuff.festering_scythe_debuff.up|!talent.festering_scythe" );
  ...
  variables->add_action( "variable,name=epidemic_prio,value=active_enemies>=3+pet.whitemane.active&!buff.forbidden_knowledge.up|active_enemies>=5+pet.whitemane.active&buff.forbidden_knowledge.up" );
}
//unholy_apl_end
