...
void unholy( player_t* p )
{
  ...
  cooldowns->add_action( "outbreak,if=(!talent.blightburst|talent.blightburst&(cooldown.putrefy.remains>gcd.max*2|time<5))&(dot.dread_plague.n_dotted_targets=0|dot.virulent_plague.n_dotted_targets=0)&fight_remains>gcd.max*2&(!raid_event.adds.exists|raid_event.adds.exists&raid_event.adds.remains>gcd.max*2)" );
