...
void fury( player_t* p )
{
  ...
  thane->add_action( "thunder_blast,if=buff.enrage.up&buff.avatar.remains<=2" ); 
  thane->add_action( "thunder_blast,if=cooldown.avatar.remains<10" );
  thane->add_action( "odyns_fury" );
  thane->add_action( "bloodbath,if=buff.scent_of_blood.stack=2&talent.vicious_contempt&target.health.pct<35" );
  thane->add_action( "rampage,if=buff.enrage.remains<gcd" );
  thane->add_action( "rampage,if=buff.recklessness.up&rage>=130" );
  thane->add_action( "thunder_blast,if=buff.enrage.up&buff.thunder_blast.stack=2" );
  thane->add_action( "bloodbath" );
  thane->add_action( "thunder_blast,if=buff.enrage.up&buff.avatar.up" );
  ...
  thane->add_action( "thunder_blast,if=buff.enrage.up&buff.avatar.up&buff.avatar.remains<4" );
  thane->add_action( "bloodthirst,if=talent.vicious_contempt&target.health.pct<35" );
  thane->add_action( "rampage,if=rage>=100" );
  thane->add_action( "execute,if=talent.executioners_wrath|talent.deep_wounds" );
