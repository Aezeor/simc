...
void devastation( player_t* p )
{
  ...
  st->add_action( "call_action_list,name=es,if=cooldown.dragonrage.up&talent.imminent_destruction&talent.mass_disintegrate", "Acquire some mass disintegrate without wasting imminent destruction time." );
  ...
  st->add_action( "tip_the_scales,use_off_gcd=1,if=(!talent.dragonrage|buff.dragonrage.up)&(cooldown.fire_breath.remains<=cooldown.eternity_surge.remains|(cooldown.eternity_surge.remains<=cooldown.fire_breath.remains&talent.font_of_magic)&!talent.engulf)", "Tip second FB if not playing font of magic, otherwise tip ES." );
  st->add_action( "deep_breath,if=talent.maneuverability&talent.melt_armor" );
  st->add_action( "dragonrage,if=(cooldown.fire_breath.remains<4|cooldown.eternity_surge.remains<4&(!talent.mass_disintegrate|set_bonus.tww2_4pc))&((cooldown.fire_breath.remains<8|!talent.animosity)&(cooldown.eternity_surge.remains<8|talent.mass_disintegrate|!talent.animosity|set_bonus.tww2_4pc))&target.time_to_die>=32|fight_remains<32" );
