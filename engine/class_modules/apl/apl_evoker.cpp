...
void devastation( player_t* p )
{
  ...
  precombat->add_action( "variable,name=es_send_threshold,value=6,if=talent.iridescence", "TWW S2 Stuff" );
  precombat->add_action( "variable,name=es_send_threshold,value=10,if=!talent.iridescence" );
  ...
  st->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=4,if=talent.scorching_embers&talent.engulf&action.engulf.usable_in<=duration+0.5", "If has scorch and the gulf is gulfable do a four empower" );
  st->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=1,if=talent.engulf", "Sad Engulf Noises (Fulminous Roar Sucks)" );
  st->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=2", "R2 funny breath good default for SC." );
  st->add_action( "eternity_surge,target_if=max:target.health.pct,empower_to=2,if=(!talent.power_swell|buff.power_swell.remains<=duration|!talent.mass_disintegrate)&active_enemies=2&!talent.eternitys_span", "Why would you do this to yourself. Talent span right meow." );
  st->add_action( "eternity_surge,target_if=max:target.health.pct,empower_to=1,if=!talent.power_swell|buff.power_swell.remains<=duration|!talent.mass_disintegrate", "Surge all of eternity and swell power (but not if :engulf:)" );
  st->add_action( "engulf,if=(dot.fire_breath_damage.remains>travel_time)&(dot.living_flame_damage.remains>travel_time|!talent.ruby_embers)&(dot.enkindle.remains>travel_time|!talent.enkindle)&(!talent.iridescence|buff.iridescence_red.up)&(!talent.scorching_embers|dot.fire_breath_damage.duration<=8|fight_remains<=30)", "Gulf :garf:" );
  ...
  st->add_action( "disintegrate,target_if=min:debuff.bombardments.remains,chain=1,if=(raid_event.movement.in>2|buff.hover.up)&!variable.pool_for_id&!variable.pool_for_cb", "Disintegrator pov" );
