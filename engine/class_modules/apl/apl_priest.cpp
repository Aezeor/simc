...
void shadow( player_t* p )
{
  ...
  cds->add_action( "dark_ascension,if=(pet.fiend.active&cooldown.fiend.remains>=4|!talent.mindbender&!cooldown.fiend.up|active_enemies>2&!talent.inescapable_torment)&(active_dot.shadow_word_madness>=1|insanity>=(20-(5*talent.minds_eye)+(5*talent.distorted_reality)-(pet.fiend.active*2)))", "Use Dark Ascension when you have enough Insanity to cast Devouring Plague." );
  ...
  main->add_action( "tentacle_slam,target_if=dot.vampiric_touch.refreshable,if=!variable.holding_tentacle_slam&!action.tentacle_slam.in_flight", "Use Tentacle Slam as long as you are not holding for adds and Vampiric Touch is within pandemic range" );
  ...
  main->add_action( "shadow_word_madness,if=buff.voidform.up&talent.void_eruption|buff.power_surge.up|talent.distorted_reality" );
