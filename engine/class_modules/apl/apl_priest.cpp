...
void shadow( player_t* p )
{
  ...
  main->add_action( "vampiric_touch,target_if=max:(refreshable*10000+target.time_to_die),if=refreshable&target.time_to_die>12", "actions.main+=/tentacle_slam,if=!variable.holding_tentacle_slam&raid_event.adds.in>=30&talent.descending_darkness&raid_event.movement.in>=30  actions.main+=/shadow_word_death,target_if=target.health.pct<(20+15*talent.deathspeaker),if=talent.depth_of_shadows  actions.main+=/shadow_word_death,target_if=min:target.time_to_die,if=talent.inescapable_torment&pet.fiend.active  Put out Vampiric Touch on enemies that will live at least 12s as a filler action." );
