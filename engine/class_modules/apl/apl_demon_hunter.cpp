...
void devourer( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=reaps,if=action.reap.souls_consumed>=4&!talent.voidfall&buff.metamorphosis.up&(!buff.moment_of_craving.up|action.reap.souls_consumed>=9|!variable.should_use_star)", "Voidscarred Reap Line" );
